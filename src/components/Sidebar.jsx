    ...
    <aside className="w-[220px] bg-[#161b27] border-r border-white/10 flex flex-col h-screen sticky top-0 flex-shrink-0">
      ...
      <div className="px-5 pt-6 pb-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[15px] font-bold text-[#e2e8f0]">AssessAI</span>
        </div>
      ...
      <div className="px-5 py-4 border-t border-white/10">
        <p className="text-[13px] font-medium text-[#e2e8f0] mb-0.5">{user?.name}</p>
        <p className="text-xs text-[#4a5568] mb-3">{user?.email}</p>
        <Button onClick={signOut} variant="ghost" size="sm" className="w-full justify-center">
          Sign out
        </Button>
      </div>
    </aside>
